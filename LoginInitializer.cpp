...
void DrvFtaeAlarm::LoginInitializer::CreateModule(HINSTANCE hInstance, std::function<ODS::UI::IAbstractUIFacrory* (void)> factoryGetter, const std::shared_ptr<UIDialogViewController>& parent, std::shared_ptr<ConnectionAttributes> attributes)
{
	std::shared_ptr<ILoginViewOutput> presenter = std::make_shared<LoginPresenter>(DataSQLServerAccessAssembly::instance().GetDataBaseEngine(), attributes);
