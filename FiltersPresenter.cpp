...
void DrvFtaeAlarm::FiltersPresenter::LoadProperties() {
	...
	if (ptrView) {
		...
		if (!_externAttributes->serverName.empty() && !_externAttributes->databaseName.empty()) {
			...
			std::unique_ptr<SQLTable> table = _databaseInfoDAO->GetTableInfoInDirectConnection(*_externAttributes, _externAttributes->databaseName, std::string("ConditionEvent"));
			if (table) {
